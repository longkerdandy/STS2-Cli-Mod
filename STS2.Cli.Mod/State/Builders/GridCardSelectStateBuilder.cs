...
/// <summary>
///     Extracts grid card selection state from <see cref="NCardGridSelectionScreen" /> subtypes.
...
///     NSimpleCardSelectScreen (generic — combat Headbutt, Hologram, events, etc.).
/// </summary>
public static class GridCardSelectStateBuilder
{
    ...
    public static GridCardSelectStateDto? Build(NCardGridSelectionScreen screen)
    {
        try
        {
            ...
            return new GridCardSelectStateDto
            {
        ...
        {
            Logger.Error($"Failed to build grid card select state: {ex.Message}");
            return null;
        }
    }
