    ...
    public static async Task<object> ExecuteAsync(Request request)
    {
        ...
        try
        {
            ...
            if (entry == null)
                return new
                {
                    ok = false, error = "ITEM_NOT_FOUND", message = $"Relic '{relicId}' (nth={nth}) not found in shop"
                };
